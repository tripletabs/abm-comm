...
def initialize():
    global g, nextg, topic_count_dict
    ...
    # intialize a topic count dictionary
    topic_count_dict = {}  # record topic counts
    for j in range(NUM_topics):
        topic_count_dict[j] = [sum([1 for n in g.nodes if g.node[n]["topic"] == j])]
    print("intial topic assignment:", topic_count_dict)
...
def update():
    ...
    # update the topic count dict
    for j in range(NUM_topics):
        topic_count_dict[j].append(sum([1 for n in g.nodes if g.node[n]["topic"] == j]))
